/**
 * @file get_feat_per_point_plugin.h
 * @brief TensorRT plugin skeleton for GetFeatPerPoint operation.
 *
 * 输入:
 *  - point_feats
 *  - voxel_count_list
...
class GetFeatPerPointPlugin final : public IPluginV2DynamicExt {
 public:
  // 用于从网络定义阶段创建插件
  explicit GetFeatPerPointPlugin(const std::string& name);

  // 用于反序列化创建插件
  GetFeatPerPointPlugin(const std::string& name, const void* data, size_t length);
...
class GetFeatPerPointPluginCreator final : public IPluginCreator {
 public:
